...
- [Utilizing the LLMSVS](#utilizing-the-llmsvs)
  ...
  - [OWASP ASVS cross-references](#owasp-asvs-cross-references)
...
**Changes in this edition (v2.0):** publication year and copyright notice are updated; front matter and table of contents are aligned with the frontispiece; editorial corrections apply across the requirement text (for example “completions” instead of “compilations,” standard hyphenation). **Technical updates** include: an [OWASP ASVS cross-references](#owasp-asvs-cross-references) subsection mapping ASVS 4.0.3 citations to ASVS 5.0.0 equivalents; clarified requirement **5.10** on safe error handling and logging versus client exposure; requirement **5.5** extended with optional structured-output guidance; clarified control objectives for **V3** (real-time learning scope), **V4** (RAG-style retrieval), and **V6** (tools and protocol-based connectors); and **Appendix A** glossary additions (indirect prompt injection, retrieval-augmented generation, tool calling). The numbered verification requirements (V1–V8) and their identifiers match the verification set carried forward from the initial 0.1 release.
...
### OWASP ASVS cross-references

Several LLMSVS requirements refer to the [OWASP Application Security Verification Standard (ASVS)](https://owasp.org/www-project-application-security-verification-standard/) for controls that apply to the wider application as well as to LLM integration. Original citations in this document use **ASVS 4.0.3** section numbering. **ASVS 5.0.0** reorganized some material; assessors should satisfy the **intent** of each LLMSVS requirement using the ASVS edition their organization has adopted. The following mapping is illustrative for **ASVS 5.0.0**:


| Topic                                                                                                                 | ASVS 4.0.3 (cited in LLMSVS)                       | ASVS 5.0.0 (illustrative equivalent)                                                               |
| --------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| Service credentials, API keys, and secret handling for integrations                                                   | Chapter **V2**, **V2.10** “Service Authentication” | Chapter **V13**, **V13.3** “Secret Management” and **V13.2** “Backend Communication Configuration” |
...
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --- | --- | --- |
| 1.1 | Identify any components that store secrets, like API keys, for third-party systems, like hosted LLMs and vector databases, and ensure the secure handling of these credentials according to OWASP ASVS requirements for service credentials and secrets (see [OWASP ASVS cross-references](#owasp-asvs-cross-references)). |     | ✓   | ✓   |
...
Establish controls to reduce the risks associated with real time learning within LLM systems, where the models are continuously fine-tuned based on user interactions in real time. This chapter addresses **ongoing or near-real-time updates to model behavior or weights** driven by live user data; it is distinct from conversational or long-term memory (see V4) and from batch or periodic training governed under V2, unless that training is applied continuously in production from user interactions at short intervals.
...
Ensure that mechanisms providing “memory” or additional knowledge not included during the training phase handle that information safely. This includes typical **retrieval-augmented generation (RAG)** patterns in which retrieved documents or embeddings are supplied as context to the model.
...
| 5.4  | Ensure that credentials for LLM providers are securely handled according to OWASP ASVS requirements for service credentials and secrets (see [OWASP ASVS cross-references](#owasp-asvs-cross-references)).                                                                                                                                                                                                                                                                                                                                                |     | ✓   | ✓   |
| 5.5  | Ensure that the output format and properties of the data returned from the LLM match the expected structure and properties. Specifically, when a response is expected in JSON, the result should not only be in valid JSON format, but also undergo schema validation to ensure it contains all the expected JSON fields and does not include any unnecessary or extraneous properties. Where the provider supports structured outputs, constrained decoding, or similar mechanisms, consider using them as defense in depth alongside schema validation. | ✓   | ✓   | ✓   |
...
| 5.10 | Ensure that client-facing responses from LLM interactions do not expose raw provider errors, stack traces, or other detailed diagnostics that could leak prompts, credentials, or internal implementation details. Present generic or sanitized messages to end users while retaining structured server-side logging for operations and security monitoring, without writing secrets or full sensitive prompts to untrusted or broadly accessible logs.                                                                                                   | ✓   | ✓   | ✓   |
| 5.11 | Ensure that appropriate LLM guards are used to scan prompts and completions to help detect potential prompt injection attacks.                                                                                                                                                                                                                                                                                                                                                                                                                            |     | ✓   | ✓   |
| 5.12 | Ensure that all prompts are considered to be untrusted and subjected to any deployed security controls. Reflecting stored data, data from third-party APIs, or the response from previous prompt completions may lead to indirect prompt injections and must be subjected to the same controls as prompts containing direct user input.                                                                                                                                                                                                                   |     | ✓   | ✓   |
| 5.13 | Ensure that the output of LLM completions is considered to be untrusted by any subsequent system. For example, if using the LLM response within a SQL query, the query should not be constructed by concatenating parts of the LLM response; follow OWASP ASVS requirements for injection prevention (see [OWASP ASVS cross-references](#owasp-asvs-cross-references)) and use parameterized queries, ORMs, or equivalent protections.                                                                                                                    | ✓   | ✓   | ✓   |
...
The autonomous nature of agent-based systems—including use of **tools**, **plugins**, and protocol-based remote connectors (for example Model Context Protocol servers)—presents new risks and can increase the impact of attacks such as prompt injection. These controls aim to reduce the risk associated with autonomous LLM components to an acceptable level.
...
| 6.5  | Ensure that credentials for third-party services consumed by agent tools and plugins are securely handled according to OWASP ASVS requirements for service credentials and secrets (see [OWASP ASVS cross-references](#owasp-asvs-cross-references)).                            |     | ✓   | ✓   |
...
- **Indirect Prompt Injection** — An attack where untrusted content reaches the model **without** being supplied directly as the end user’s current message—for example via retrieved documents, tool outputs, email bodies, web pages, or stored data that is later included in context—causing the model to follow attacker-chosen instructions or to leak or misuse data.
...
- **Tool Calling (Function Calling)** — A pattern where the model emits structured requests to invoke application-defined or third-party capabilities (tools, APIs, or functions), which execute outside the model and whose results may be fed back into the conversation. Security depends on validation, authorization, and least privilege for each tool.
...
- **Retrieval-Augmented Generation (RAG)** — An architecture where relevant documents or passages are retrieved (often from a vector store or search index) and provided as context to an LLM to improve factual grounding. RAG introduces supply-chain, access-control, and prompt-injection risks in the retrieval path and in how retrieved text is combined with user input.
